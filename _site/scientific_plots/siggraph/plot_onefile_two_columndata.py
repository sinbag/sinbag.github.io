...
def plot_data(inputfilename, outputfilename,folder='./'):
    data = readTXT(os.path.join(folder, inputfilename))
    
    X = data[:,0]
    Y = data[:,1]
    print(Y)
    print('xlimit: ',np.min(X),np.max(X))
    print('ylimit: ',np.min(Y),np.max(Y))
    ...
    line1 = ax.plot(np.log2(X),np.log2(Y))
    
    plt.xscale('log')
#    plt.yscale('log')
    plt.savefig(outputfilename,bbox_inches='tight',pad_inches=0.05)
    plt.close()
...
def main(argv):
    ...
    plot_data(inputfile,outputfile)
